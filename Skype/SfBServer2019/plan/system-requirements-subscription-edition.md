---
ms.date: 07/07/2025
title: "System requirements for Skype for Business Server Subscription Edition"
ms.reviewer: 
ms.author: heidip
...
> [!NOTE]
> The system requirements mentioned in this topic apply to both Skype for Business Server Subscription Edition and Skype for Business Server 2019.
...
|:-----|:-----|
|CPU   |Intel Xeon E5-2673 v3 dual processor, 6-core, 2.4 gigahertz (GHz) or greater.  <br/> Intel Itanium processors aren't supported for Skype for Business Server SE roles.   |
...
> [!NOTE]
> Regardless of the server role, we also recommend the following hardware settings for Skype for Business Server SE (the settings might vary depending on the brand of hardware you purchase, so refer to manufacturer documentation for specifics):
...
Anything other than the operating systems that are listed here won't work correctly. Don't try to use anything else for installations of Skype for Business Server SE. For example, Server Core option isn't listed. Therefore, it's not supported.

> [!NOTE]
> - Windows Server 2022 qualifies only with Skype for Business Server SE for Cumulative Update 7 and later versions (minimum Build Number 2046.524).
> 
...
There are some things that you must install or configure for any server that's running Skype for Business Server SE. These things are listed in the following tables, followed by other requirements for specific server roles.

> [!IMPORTANT]
> - Skype For Business 2019 supports .NET Framework 4.8. and .NET Framework 4.8.1
>
> - Skype for Business Server SE requires Visual C++ redistributable package for Visual Studio 2012 and Visual C++ redistributable package for Visual Studio 2013, which are automatically installed as part of the Skype for Business Server SE installation.
>
...
|Windows Identity Foundation   |We need Windows Identity Foundation 3.5 to support server-to-server authentication scenarios for Skype for Business Server SE.  <br/> <li> For Windows Server 2016, there's no need to download anything. Open **Server Manager**, and go to the **Add Roles and Features Wizard**. **Windows Identity Foundation 3.5** is listed under the **Features** section. If it's selected, all set. Otherwise, select it, and then select **Next** to reach the **Install** button.  |
...
|Windows Media Format Runtime   | For Windows Server 2016, you have to install the **Media Foundation** feature in **Server Manager**. You actually can start your Skype for Business Server SE installation without this feature. However, you're prompted to install it, and then restart the server before the Skype for Business Server SE installation can continue. It's better to do it ahead of time.  |
|Silverlight   |You can install the latest version of Silverlight [here](https://www.microsoft.com/silverlight/).   |
|For users in China region |[Run PowerShell script](disable-legacy-control-panel.md#disable-legacy-control-panel-cscp) after you update the server to the minimum build number Skype for Business Server SE Cumulative Update 7 Hotfix 1 (2046.524).   |
...
Starting with Skype for Business 2019 CU8, Skype for Business Server SE Enterprise Edition also supports SQL Server 2022 Express (64-bit edition) for local SQL express instances. This can also be updated by an in place upgrade of SQL Express.

Skype for Business Server SE Enterprise Edition requires the full version of SQL Server, as indicated here (only 64-bit edition; don't use 32-bit editions):

- Microsoft SQL Server 2022 (64-bit edition) - must be run together with the latest updates, starting with Skype for Business Server SE CU8
...
SQL Clustering with Skype for Business Server SE is supported. If you want to set up SQL Clustering, that's done in SQL Server.
...
Two-node:
- Microsoft SQL Server 2022 Standard (64-bit edition), and we recommend running with the latest service pack, starting with Skype for Business Server SE CU8.  
...
Sixteen-node:
- Microsoft SQL Server 2022 Enterprise (64-bit edition), and we recommend running with the latest service pack, starting with Skype for Business Server SE CU8.
...
SQL Always On is supported, and you can read more about it in [Back End Server high availability in Skype for Business Server SE](../../SfbServer/plan-your-deployment/high-availability-and-disaster-recovery/back-end-server.md).
...
The domain functional level of any domain that you deploy Skype for Business Server SE into, and the forest functional level of any forest that you deploy Skype for Business Server SE into, must be one of the following:
...
It's important to know that Skype for Business Server SE doesn't support single-labeled domains. What are these? If you have a root domain that's labeled "contoso.local," that works. If you have a root domain that's named "local," that won't work, and it's not supported. For more information, see [Deployment and operation of Active Directory domains that are configured by using single-label DNS names](/troubleshoot/windows-server/identity/deployment-operation-ad-domains).
  
Skype for Business Server SE also doesn't support renaming domains. If you must rename your domain, you have to uninstall Skype for Business Server SE, do the domain rename, and then reinstall Skype for Business Server SE.
  
Finally, you may be dealing with a domain that has a locked-down AD DS environment, and that's acceptable. We have more information about how to deploy Skype for Business Server SE into a locked-down AD DS environment in the Deployment documentation.
  
### AD Topologies

Supported topologies in Skype for Business Server SE are as follows:
...
We have diagrams and descriptions to help you determine what topology you have in your environment, or what you may have to set up prior to installing Skype for Business Server SE. To keep it simple, we're also including a key:
...
Skype for Business Server SE does support multiple forests configured in a central forest topology. If you're not sure that's what you have, the central forest in the topology uses objects in it to represent users in the other forests, and hosts user accounts for any users in the forest.
...
A resource forest topology is also supported; it's where a forest is dedicated to running your server applications, like Microsoft Exchange Server and Skype for Business Server SE. This resource forests also hosts a synchronized representation of active user objects, but no logon-enabled user accounts. So the resource forest is a shared services environment for other forests in which user objects reside, and they have a forest-level trust relationship with the resource forest.
...
- DNS enables Skype for Business Server SE to discover internal servers or pools, allowing for server-to-server communications.
...
It's important to note that Skype for Business Server SE doesn't support internationalized domain names (IDNs).
  
And it's important to remember that any name in DNS should be identical to the computer name configured on any server that is used by Skype for Business Server SE. Specifically, we can't have any short-names in the environment, and must have FQDNs for Topology Builder.
...
- All certificates must be signed using a signing algorithm supported by the operating system. Skype for Business Server SE supports the SHA-1 and SHA-2 suite of digest sizes (224-bit, 256-bit, 384-bit and 512-bit), and meets or exceeds the operating system requirements.
    
- Auto-enrollment is supported for internal servers running Skype for Business Server SE.
    
- Auto-enrollment isn't supported for Skype for Business Server SE Edge Servers.
  ...
  > [!NOTE]
  > Skype for Business Server SE does not support CNG certificates.
...
You'll also need certificates when Skype for Business Server SE communicates with other applications and servers, such as Microsoft Exchange Server. This will, obviously, have to be a certificate that these other apps and servers can use in a supported way. Skype for Business Server SE and other Microsoft products support the Open Authorization (OAuth) protocol for server-to-server authentication and authorization. If you're interested, we have other planning article for OAuth and Skype for Business Server SE.
  
Skype for Business Server SE also includes support for (without requiring) certificates signed using the SHA-256 cryptographic hash function. To support external access using SHA-256, the external certificate needs to be issued by a public CA using SHA-256.
...
**Certificates for Survivable Branch Appliance (Specifically, Survivable Branch Appliance 2015 for Skype for Business Server SE)**
...
This is where a little pre-planning is good. But sometimes, you've deployed Skype for Business Server SE without intending to deploy mobility, and that comes up later when you already have certificates in your environment. Reissuing the certificates through an internal CA is typically easy. But for public certificates from a public CA that can be a little more pricey.
...
Skype for Business Server SE can use the same file share for all file storage. You should keep the following in mind:

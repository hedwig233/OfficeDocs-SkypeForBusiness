...
|3P meetings      |Yes (Cisco WebEx and Zoom)         |No         |

## Where is this in Pro Portal?

To navigate to the **Shared Spaces Insights (Preview)** page in the Pro Management Portal, perform the following steps:
...
1. Select the desired location, business hours and days, and date filters by choosing values from the following dropdown lists:
   ...
   |Dropdown list name  | Value chosen to determine which attribute  |
   |---------|---------|
...
1. Select **Apply**.
   ...
   > [!NOTE]
   > The IT admin can view the utilization metrics of only those shared spaces that they are authorized to view, based on the roles assigned to you. For more information about the roles assigned to you and the shared spaces for which you're  
   authorized to view the utilization metrics, see [Role-Based Access Control (RBAC)](#role-based-access-control-rbac). 

### Page components

The **Shared Spaces Insights (Preview)** page contains the following components the user must use to access the final utilization metrics. These components are different types as described in the following table:

|Component Name  |Type  |
|---------|---------|
|[Global page filters](#global-page-filters)     |Dropdown lists from which the user must choose a value.         |
|[Shared spaces utilization percentage](#shared-spaces-utilization-percentage)     | A chart that the user can view.          |
|[Peak utilization chart](#peak-utilization-chart)     |A chart that the user can view.         |
|[Overall shared spaces utilization](#overall-shared-spaces-utilization)     |A chart that the user can view.         |
|[Most and least utilized rooms](#most-and-least-utilized-rooms)     |A chart that the user can view.         |
...
- **Location Discovery**: This filter's pane is located on the top of the page at which it lets you filter the data for your preferred location.
...
- **Days of the week and business hours filter**: "Days" and "business hours" are the other attributes for which you can choose values to customize the insights to your organization's schedule.
...
Data tiles are the tiles below the dropdown lists. These tiles display the utilization data for the groups you choose from the dropdown list on the top–left side of the **Shared Spaces Insights (Preview)** page (next to the portal's name).

The following screenshot depicts the example of a data tile:

:::image type="content" source="../media/data-tiles.png" alt-text="Screenshot that shows a data tile and the information it displays on the Shared Spaces Insights (Preview) page." lightbox="../media/data-tiles.png":::
...
- **Total capacity**: This tile displays summed-up value of the capacity ([capacity property in the room resource account](/graph/api/resources/room?view=graph-rest-1.0)) of all the Teams Rooms based on the locations selected. If the capacity information isn't available, it's excluded.

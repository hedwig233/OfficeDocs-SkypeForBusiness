...
|Detect sensitive content during screen sharing|Can be set to **On** or **Off**. If set to **Off**, then not available in meeting options.|Can enforce or prevent notifications for detective content during screen sharing or leave the setting uncontrolled. Can sensitive content detection even if the admin policy is **Off** for the organizer.|Can enforce or prevent notifications for detective content during screen sharing unless a sensitivity label controls the setting. Setting can be enforced or meeting organizer can be allowed to change.|Meeting organizer can turn **On** or **Off** unless the setting is disabled by admin policy or enforced **On** or **Off** by a meeting template or sensitivity label.|
